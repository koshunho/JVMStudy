...
![JVM体系结构](https://raw.githubusercontent.com/koshunho/koshunhopic/master/2.png)
 所谓JVM调优，基本上调的就是堆。（方法区是特殊的堆,JDK1.8之后在元空间里）
 
## 2.  类加载器ClassLoader
#### 类的加载、连接、初始化
![加载过程](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E5%8A%A0%E8%BD%BD%E8%BF%87%E7%A8%8B.png)
 - 加载：查找并加载类的二进制数据
...
#### ClassLoader分类
![ClassLoader分类](https://raw.githubusercontent.com/koshunho/koshunhopic/master/1.png)
...
![栈帧](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E6%A0%88%E5%B8%A7.png)
...
![堆内存逻辑](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E5%A0%86%E5%86%85%E5%AD%98%E9%80%BB%E8%BE%91.png)


GC主要是在新生区和老年区，又分为**轻GC** 和 **重GC**，如果内存不够or存在死循环，就会导致 OOM错误

![堆结构](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E5%A0%86%E7%BB%93%E6%9E%84.png)
...
元空间(Metaspace)鸠占鹊巢，方法区存在于元空间(Metaspace)。同时，JDK1.8元空间不再与堆连续，而且是存在于**本地内存（Native memory）**
...
![VM参数调优](https://raw.githubusercontent.com/koshunho/koshunhopic/master/VM%E5%8F%82%E6%95%B0%E8%B0%83%E4%BC%98.png)

PSYoungGen      total 305664K +  ParOldGen       total 699392K = 1005056/1024 =981.5MB

也就证明了元空间并不在虚拟机，而是使用本地内存。
==>元空间逻辑上存在于堆内存，但物理上不存在于堆内存
...
![结果1](https://raw.githubusercontent.com/koshunho/koshunhopic/master/GC1.png)

![结果2](https://raw.githubusercontent.com/koshunho/koshunhopic/master/GC2.png)
...
#### 引用计数法
![引用计数法](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E5%BC%95%E7%94%A8%E8%AE%A1%E6%95%B0%E6%B3%95.png)
...
![复制算法](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E5%A4%8D%E5%88%B6%E7%AE%97%E6%B3%95.png)

年轻代分成三个部分：一个Eden区和2个Survivor区（from区，to区）。默认比例8:1:1

因为年轻代的对象基本上都是**朝生夕死**，所以年轻代的垃圾算法使用的是复制算法。复制算法的思想就是将内存分成两块，每次只用一块，当这一块内存用完，就将还活着的对象复制到另外一块上面。复制算法不会产生内存碎片。
...
![标记清除算法](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E6%A0%87%E8%AE%B0%E6%B8%85%E9%99%A4.png)
...
#### 标记压缩算法
![标记压缩算法](https://raw.githubusercontent.com/koshunho/koshunhopic/master/%E6%A0%87%E8%AE%B0%E5%8E%8B%E7%BC%A9.png)

在压缩阶段，不再对标记的对象作回收，而是通过所有存活对象都向一端移动，然后**直接清除边界以外的内存**。可以看到，标记的存活对象将会被整理，按照内存地址依次排列，而未被标记的内存会被清理掉。
...
只要使用 new 方法，便需要创建新的对象。使用双引号就会直接存在字符串常量池，**没有双引号的都会在堆！**。

特别注意String str4 = str1 + str2; 
字符串拼接生成的对象只在**堆**，不会在常量池中！

原理：其实是因为当我们使用变量拼接的时候，java会帮我们优化成StringBuilder对象进行append,最后在调用toString返回一个对象给我们，所以常量池中是没有这个字符串对象的。
```java
